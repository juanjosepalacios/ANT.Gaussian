...
1013 format(7f9.4)
  ...
  subroutine MullPop_SOC
    ...
    IF( NSpin == 2 ) THEN 
      ...
      rho_ab_I = matmul( PD_SOC_UU, S_SOC_UD )+matmul( PD_SOC_UD_I, S_SOC_DD )
    ...
    do j=1,NALead(1)
       ...
       IF(NSpin ==2 ) THEN     
         spindens = sqrt((ro_ab+ro_ba)**2+(0.5d0*(ro_ab-ro_ba))**2+(ro_a-ro_b)**2)
         write(ifu_log,2012)'Atom:',j,' El.dens:',(ro_a+ro_b),' Sp.dens.x:',(ro_ab+ro_ba),' Sp.dens.y:',(0.5d0*(ro_ab-ro_ba)),' Sp.dens.z:',(ro_a-ro_b),' Coll. sp.dens:',spindens
       END IF
       IF(Mulliken .and. LDOS_Beg <= LDOS_End) THEN
         if(NSpin ==1 ) write(ifu_mul,2013)(GetAtmCo(n,j)*Bohr,n=1,3),ro_a*sdeg,AtomDOSEF(1,j)
         if(NSpin ==2 ) write(ifu_mul,2013)(GetAtmCo(n,j)*Bohr,n=1,3),(ro_a+ro_b),(ro_ab+ro_ba),(0.5d0*(ro_ab-ro_ba)),(ro_a-ro_b),(AtomDOSEF(l,j)*(-1)**(l+1),l=1,2)
       END IF
       IF(Mulliken .and. LDOS_Beg > LDOS_End) THEN
         if(NSpin ==1 ) write(ifu_mul,2013)(GetAtmCo(n,j)*Bohr,n=1,3),ro_a*sdeg
         if(NSpin ==2 ) write(ifu_mul,2013)(GetAtmCo(n,j)*Bohr,n=1,3),(ro_a+ro_b),(ro_ab+ro_ba),(0.5d0*(ro_ab-ro_ba)),(ro_a-ro_b)
       END IF
       chargelead1=chargelead1+(ro_a+ro_b)

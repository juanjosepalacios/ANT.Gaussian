...
!!    subroutine CompDensMat3(ADDP)
!!       use numeric, only: Secant, Muller, BISEC,Rtrace,RMatPow,CInvo,RSDiag
!!   !    use g09Common, only: GetNAE, GetNBE
!!       use parameters, only:
!!   FermiAcc,ChargeAcc,Max,QExcess,QMolEx,QEl1Ex,QEl2Ex,OAS!,MISTEP
!!       use parameters, only: SubSpBeg,SubSpEnd,AORB,OLDPS,ISU,ISD,Inttyp
!!       use parameters, only: biasvoltage,INTPD
!!       use cluster, only: hiaorbno, loaorbno
!!       use constants
!!       use MolMod, only: OrthProjMatMix,OrthProjMat,Red_Den,PALSORB,PALSORMix
!!       use preproc, only: MaxAtm
!!     USE g09Common, ONLY: GetNAE, GetNBE,GetNAtoms,GetNShell, GetAtm4Sh,
!!   Get1stAO4Sh, GetNBasis, GetAN
!!   
!!   !    use ieee_arithmetic
!!       implicit none
!!   
...
!!       real*8 :: E0,E1,E2,E3,DE,Z, Delta, Epsilon
!!   
!!       logical :: root_fail,root_ermi
!!   
!!       integer :: MBeg,MEnd,Mdim,El1dim,El2dim,NMcyc,info
!!       real*8, dimension(:,:), allocatable :: PDEl1,PDEl2,PDMoltest,SMol,ShalfMol
!!       real*8, dimension(:,:), allocatable :: PDMol,SDMol
!!       real*8, dimension(NAOrbs,NAOrbs) ::
!!   PDtemp1,PDtemp2,PDsave,RSInv,PDold,PDFroztemp
!!       real*8, dimension(Nspin,NAOrbs,NAOrbs) :: PDtemp2Sp
!!       complex*16, dimension(NAOrbs,NAOrbs) :: CSInv
!!       real*8, dimension(NAOrbs) :: Eigen
!!       real*8 :: QMol0,QEl10,QEl20,Fermi1,Fermi2,Fermi3
!!       real*8 :: QMol0b,QEl10b,QEl20b,Fermi1b,Fermi2b,Fermi3b
!!       integer :: iatom,ii,jatom,jj,ntot,M
!!       INTEGER, DIMENSION(MaxAtm) :: NAO
!!       real*8 :: rrr, a, b, Q
!!   
!!   
!!   !call Red_Den(HD,PD,SD,nel)
!!   !STOP
!!       Z=10.0d0*FermiAcc
!!       Delta=FermiAcc
!!       Epsilon=ChargeAcc*(NCDEl+QExcess+QEl1Ex+QEl2Ex)
!!       root_ermi = .true.
!!       shiftup = chempota
!!       shiftdown = chempotd
!!   
...
!!       if (Mbeg > 1) then
!!         El1dim = Mbeg-1
!!         El2dim = NAOrbs - Mdim -El1dim
!!         allocate(PDMol(mdim,mdim),SDMol(mdim,mdim),PDEl1(El1dim,El1dim),PDEl2(El2dim,El2dim))
!!       else
!!         El1dim = 0
!!         El2dim = NAOrbs - Mdim -El1dim
!!         allocate(PDMol(mdim,mdim),PDEl2(El2dim,El2dim),SDMol(mdim,mdim))
!!       end if
!!   
!!       QMol0 = d_zero
!!       QEl10 = d_zero
...
!!       ntot = ntot +1
!!   
!!       if (OAS) shift = shiftFIX
!!   
!!   !Compute QStepF integrating into the molecular subspace
!!          root_fail = .true.
!!   !       shift = chempotemp
!!          E0=shift-z!shiftm-Z
!!          E1=shift
!!          E2=shift+z!m+Z
!!          if (root_fail) then
!!             print*,'MULLER method in QStepF. Molecular Step.'
!!             call MULLER(QStepF,E0,E1,E2,Delta,Epsilon,Max,E3,DE,K,Cond)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E2) .or. ieee_is_nan(E2) .or.
!!   !          E2<EMin .or. E2>EMax) then
!!             if(k .eq. Max .or. E2<EMin .or. E2>EMax) then
!!                print *, 'Warning: MULLER method failed to find root. Using
!!   SECANT.'
!!                root_fail = .true.
!!             else
!!                shift = E3
...
!!          end if
!!          if (root_fail) then
!!             print*,'SECANT method in QStepF. Molecular Step.'
!!             call SECANT(QStepF,E0,E2,Delta,Epsilon,Max,E3,DE,Cond,K)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E3) .or. ieee_is_nan(E3) .or.
!!   !          E3<EMin .or. E3>EMax) then
!!             if(k .eq. Max .or.E3<EMin .or. E3>EMax) then
!!                print *, 'Warning: SECANT method failed to find root. Using BISEC.'
!!                root_fail = .true.
!!             else
...
!!          end if
!!          if (root_fail) then
!!             print *, 'BISEC method in QStepF. Molecular Step.'
!!             shift = BISEC(QStepF,EMin,EMax,Delta,5*Max,K)
!!             DE=Delta
!!             if(k.lt.5*Max) root_fail = .true.
!!             if(k.ge.5*Max) print *, 'Warning: BISECT method failed to find root.
!!   Skipping this cycle.'
!!          end if
!!   
!!          chempot = shift
...
!!          shift = shiftemp
!!          if (OAS) shift = shiftFIX
!!   !       if (OAS) shiftemp = shiftFIX
!!          root_fail = .true.
!!          write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Molecular Step
!!   =',-Chempot,'+/-',dabs(DE), "at cyc = ", ntot
!!   
!!          call printCH
!!   
!!          IF (OAS) then
...
!!            PDFroztemp = PDFroz(1,:,:)
!!            if (OLDPS) then
!!              call OrthProjMatMix(SD,PDtemp2,PDFroztemp,NAOrbs)
!!            else
!!              call PALSORMix(SD,PDtemp2,PDFroztemp,PalSorVector)
!!            end if
!!    
!!            PD(1,:,:) = PDtemp2
!!            call PrintCh
...
!!          end if
!!   
!!   !       write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Shift
!!   !       =',-shift,'+/-',dabs(DE), "at cyc = ", ntot
!!   
!!          IF (OAS) then
!!   
!!            !Only active Space. 
...
!!   !       ENd if ! OAS
!!   
!!          Fermi2 = dabs(shift-shiftemp)
!!          if (OAS) Fermi2 = dabs(shift-shiftFIX)
!!   
!!          if (Fermi2 < 10*FermiAcc) then
!!   
!!            Mcyc = 1
!!            print*, "Fermi2 == 0", Fermi2
!!            shiftemp = shift
!!            if (OAS) shift = shiftFIX
!!            chempotemp = chempot
!!   
...
!!   !         root_fail = .true.
!!            root_fail = .false.
!!   !         Mcyc = 0 !No sirva para nada
!!            Mcyc = 1
!!   !         print*, "STEP fINAL = ", chempot, " Delta =  ", Fermi2
!!   !         print*, "Fermi fINAL = ", shift,shiftemp
!!            write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Shift
!!   =',-shift,'+/-',dabs(Fermi2), "at cyc = ", ntot
!!   
!!            shiftemp = shift
!!            if (OAS) shift = shiftFIX
!!            chempotemp = chempot
...
!!       If (OAS ) then
!!       write(ifu_log,*)'-----------------------------------------------'
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'OAS Molecular STEP = ',-ChemPotemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'OAS Fermi SHIFT = ',-Shiftemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,*)'-----------------------------------------------'
!!       Else
!!       write(ifu_log,*)'-----------------------------------------------'
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'CDFT Molecular STEP = ',-ChemPotemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'CDFT Fermi SHIFT = ',-Shiftemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,*)'-----------------------------------------------'
!!       end if
!!   
!!   
!!   !    else if (NSpin == 2 .and. FIXESP) then
!!   
!!       else if (NSpin == 2) then
!!   !    if (shiftcyc == 1) SPINCHO = .true.
!!   !    print*, "SPINCHO = ", SPINCHO
!!   !    print*, "isu,isd = ", isu,isd
!!   
!!   !    do ispin = 1,Nspin
!!   
!!       Mcyc = 0
...
!!       do ispin = 1,Nspin
!!   
!!   if (ispin == 1 .and. ISU == 0.0d0) go to 1500
!!   if (ispin == 2 .and. ISD == 0.0d0) go to 1500
!!   
!!   !Compute QStepF integrating into the molecular subspace
!!   print*, "shiftup = ", shiftup,shiftdown
!!          root_fail = .true.
!!   !       shift = chempotemp
!!             if (ispin.eq.1) E1=shiftup
!!             if (ispin.eq.2) E1=shiftdown
...
!!   !       E1=shift
!!          E2=E1+z!m+Z
!!          if (root_fail.and.intTyp /= 0) then
!!             print*,'MULLER method in QStepX. Molecular Step.'
!!             call MULLER(QStepX,E0,E1,E2,Delta,Epsilon,Max,E3,DE,K,Cond)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E2) .or. ieee_is_nan(E2) .or.
!!   !          E2<EMin .or. E2>EMax) then
!!             if(k .eq. Max .or. E2<EMin .or. E2>EMax) then
!!                print *, 'Warning: MULLER method failed to find root. Using
...
!!          end if
!!          if (root_fail.and. intTyp /=0) then
!!             print*,'SECANT method in QStepX. Molecular Step.'
!!             call SECANT(QStepX,E0,E2,Delta,Epsilon,Max,E3,DE,Cond,K)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E3) .or. ieee_is_nan(E3) .or.
!!   !          E3<EMin .or. E3>EMax) then
!!             if(k .eq. Max .or.E3<EMin .or. E3>EMax) then
!!                print *, 'Warning: SECANT method failed to find root. Using BISEC.'
...
!!          end if
!!          if (root_fail) then
!!             print *, 'BISEC method in QStepX. Molecular Step.'
!!             if (ispin.eq.1) shiftup = BISEC(QStepX,EMin,EMax,Delta,5*Max,K)
!!             if (ispin.eq.2) shiftdown = BISEC(QStepX,EMin,EMax,Delta,5*Max,K)
!!             DE=Delta
!!             if(k.lt.5*Max) root_fail = .true.
!!             if(k.ge.5*Max) then
!!               print *, 'Warning: BISECT method failed to find root. Skipping this
!!   cycle.'
!!        !       if (ispin.eq.1) E1 = shiftup - 1.0d0
!!        !       go to 1234
!!             end if
!!          end if
!!   
...
!!   !       if (OAS) shiftemp = shiftFIX
!!          root_fail = .true.
!!          if (ispin.eq.1) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Sub-Space
!!   Step (UP) =',-Chempota,'+/-',dabs(DE), "at cyc = ", ntot
!!          if (ispin.eq.2) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Sub-Space
!!   Step (DOWN) =',-Chempotd,'+/-',dabs(DE), "at cyc = ", ntot
!!   
!!   !1500 continue
!!   end do !ispin
!!          call printCH
...
!!   !       if (OAS) shiftemp = shiftFIX
!!          root_fail = .true.
!!          if (ispin.eq.1) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Metal Step
!!   (UP) =',-shifta,'+/-',dabs(DE), "at cyc = ", ntot
!!          if (ispin.eq.2) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Metal Step
!!   (DOWN) =',-shiftd,'+/-',dabs(DE), "at cyc = ", ntot
!!   
!!   !1500 continue
!!   end do !ispin
!!   !       call printCH
...
!!          E2=E1+z!m+Z
!!          if (root_fail) then
!!             print*,'MULLER method in QStepX2'
!!             call MULLER(QStepX2,E0,E1,E2,Delta,Epsilon,Max,E3,DE,K,Cond)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E2) .or. ieee_is_nan(E2) .or.
!!   !          E2<EMin .or. E2>EMax) then
!!             if(k .eq. Max .or. E2<EMin .or. E2>EMax) then
!!                print *, 'Warning: MULLER method failed to find root. Using
...
!!   !             if (ispin.eq.2) shiftdown = E3
!!                shift = E3
!!   if (ISU == 0.0d0) shiftup = E3
!!   if (ISU == 0.0d0) chempota = E3
!!   if (ISD == 0.0d0) shiftdown = E3
!!   if (ISD == 0.0d0) chempotd = E3
!!                root_fail = .false.
!!             end if
!!          end if
!!          if (root_fail) then
!!             print*,'SECANT method in QStepX2'
!!             call SECANT(QStepX2,E0,E2,Delta,Epsilon,Max,E3,DE,Cond,K)
!!   !          if(k .eq. Max .or. .not. ieee_is_finite(E3) .or. ieee_is_nan(E3) .or.
!!   !          E3<EMin .or. E3>EMax) then
!!             if(k .eq. Max .or.E3<EMin .or. E3>EMax) then
!!                print *, 'Warning: SECANT method failed to find root. Using BISEC.'
...
!!          end if
!!          if (root_fail) then
!!             print *, 'BISEC method in QStepX2'
!!   !          if (ispin.eq.1) shiftup = BISEC(QStepX,EMin,EMax,Delta,5*Max,K)
!!   !          if (ispin.eq.2) shiftdown = BISEC(QStepX,EMin,EMax,Delta,5*Max,K)
!!             shift = BISEC(QStepX2,EMin,EMax,Delta,5*Max,K)
!!   if (ISU == 0.0d0) shiftup = shift
!!   if (ISU == 0.0d0) chempota = shift
!!   if (ISD == 0.0d0) shiftdown = shift
!!   if (ISD == 0.0d0) chempotd = shift
!!             DE=Delta
!!             if(k.lt.5*Max) root_fail = .false.
!!             if(k.ge.5*Max) print *, 'Warning: BISECT method failed to find root.
!!   Skipping this cycle.'
...
!!   
!!   !do ispin=1,Nspin
!!   !       if (ispin == 1) Fermi2 = dabs(shiftup-shiftemp)
!!   !       if (ispin == 2) Fermi2 = dabs(shiftdown-shiftempd)
!!   
!!          Fermi2 = dabs(shift-shiftemp)
!!   
!!          if (OAS) Fermi2 = dabs(shift-shiftFIX)
...
!!            Mcyc = 1
!!            print*, "Fermi2 == 0", Fermi2
!!            !if (ispin == 1) shiftemp = shiftup
!!            shiftemp = shift
!!   !         if (ispin == 2) shiftempd = shiftdown
!!            if (OAS) shift = shiftFIX
!!            chempotempa = chempota
!!            chempotempd = chempotd
!!            shiftatemp = shifta
!!            shiftdtemp = shiftd
!!   
!!          else
...
!!   !         print*, "STEP fINAL = ", chempot, " Delta =  ", Fermi2
!!   !         print*, "Fermi fINAL = ", shift,shiftemp
!!   !         if (ispin ==1 ) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Shift
!!   !         UP =',-shiftup,'+/-',dabs(Fermi2), "at cyc = ", ntot
!!   !         if (ispin ==2 ) write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Shift
!!   !         UP =',-shiftdown,'+/-',dabs(Fermi2), "at cyc = ", ntot
!!             write(ifu_log,'(A,F9.5,A,f9.5,A,I6)') 'Initial Shift
!!   =',-shift,'+/-',dabs(Fermi2), "at cyc = ", ntot
!!   
!!   !         if (ispin ==1) shiftemp = shiftup
...
!!            shiftemp = shift
!!            if (OAS) shift = shiftFIX
!!   !         if (ispin ==1) chempotemp = chempot
!!   !         if (ispin ==2) chempotempd = chempotd
!!    chempotempa = chempota
!!    chempotempd = chempotd
!!    shiftatemp = shifta
!!    shiftdtemp = shiftd
...
!!       If (OAS ) then
!!       write(ifu_log,*)'-----------------------------------------------'
!!       write(ifu_log,'(A,F9.5,F9.5,A,f9.5)') 'OAS Sub-Space STEP =
!!   ',-ChemPotempa,-ChemPotempd,' +/- ',dabs(DE)
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'OAS Fermi SHIFT = ',-Shiftemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,*)'-----------------------------------------------'
!!       Else
!!       write(ifu_log,*)'-----------------------------------------------'
!!       write(ifu_log,'(A,F9.5,F9.5,A,f9.5)') 'CDFT Sub-Space STEP (UP/down) =
!!   ',-ChemPotempa,-ChemPotempd,' +/- ',dabs(DE)
!!       write(ifu_log,'(A,F9.5,F9.5,A,f9.5)') 'CDFT Metal STEP (UP/down)
!!   =',-shiftatemp,-shiftdtemp,' +/- ',dabs(DE)
!!       write(ifu_log,'(A,F9.5,A,f9.5)') 'CDFT Fermi SHIFT = ',-Shiftemp,' +/-
!!   ',dabs(DE)
!!       write(ifu_log,*)'-----------------------------------------------'
!!       end if
...
!!       ADDP = .not. root_fail
!!   
!!   !Integrate the reduced greens function
!!   !    PDMol = PD(ispin,Mbeg:Mend,Mbeg:Mend)
!!   !Print*, "IntPDOS(Emin,0d0) PS"
!!   !    call IntPDOS(Emin,0d0)
!!   !Print*, "IntPDOS(Emin,Emax) PS"
!!   !    call IntPDOS(Emin,Emax)
!!   if ( INTPD .and. shiftcyc > 900 ) then
!!   !go to 4444
!!   Print*, "IntPDOS2(Emin,0d0,1)"
!!       call IntPDOS2(Emin,0d0,1)
!!   Print*, "IntPDOS2(Emin,Emax,1)"
!!       call IntPDOS2(Emin,Emax,1)
!!   Print*, "IntPDOS2(Emin,0d0,2)"
!!       call IntPDOS2(Emin,0d0,2)
!!   Print*, "IntPDOS2(Emin,Emax,2)"
!!       call IntPDOS2(Emin,Emax,2)
!!   !SD = (NDim/NDim2)*SD
!!   !Print*, "IntPDOS2(Emin,0d0,2) - NDim "
!!   !    call IntPDOS2(Emin,0d0,2)
!!   !SD = (NDim2/NDim)*SD
!!   !Print*, "IntPDOS2(Emin,0d0,1) - Ndim recovery"
!!   !    call IntPDOS2(Emin,0d0,1)
!!   Print*, "IntPDOS2(Emin,0d0,3)"
!!       call IntPDOS2(Emin,0d0,3)
!!   Print*, "IntPDOS2(Emin,Emax,3)"
!!       call IntPDOS2(Emin,Emax,3)
!!   Print*, "IntPDOS2(Emin,0d0,4)"
!!       call IntPDOS2(Emin,0d0,4)
!!   Print*, "IntPDOS2(Emin,Emax,4)"
!!       call IntPDOS2(Emin,Emax,4)
!!   Print*, "IntPDOS2(Emin,0d0,5)"
!!       call IntPDOS2(Emin,0d0,5)
!!   Print*, "IntPDOS2(Emin,Emax,5)"
!!       call IntPDOS2(Emin,Emax,5)
!!   Print*, "IntPDOS2(Emin,0d0,6)"
!!       call IntPDOS2(Emin,0d0,6)
!!   Print*, "IntPDOS2(Emin,Emax,6)"
!!       call IntPDOS2(Emin,Emax,6)
!!   Print*, "IntPDOS2(Emin,0d0,7)"
!!       call IntPDOS2(Emin,0d0,7)
!!   Print*, "IntPDOS2(Emin,Emax,7)"
!!       call IntPDOS2(Emin,Emax,7)
!!   Print*, "IntPDOS2(Emin,0d0,8)"
!!       call IntPDOS2(Emin,0d0,8)
!!   Print*, "IntPDOS2(Emin,Emax,8)"
!!       call IntPDOS2(Emin,Emax,8)
!!   Print*, "IntPDOS2(Emin,0d0,9)"
!!       call IntPDOS2(Emin,0d0,9)
!!   Print*, "IntPDOS2(Emin,Emax,9)"
!!       call IntPDOS2(Emin,Emax,9)
!!   Print*, "IntPDOS2(Emin,0d0,10)"
!!       call IntPDOS2(Emin,0d0,10)
!!   Print*, "IntPDOS2(Emin,Emax,10)"
!!       call IntPDOS2(Emin,Emax,10)
!!   end if
!!   go to 1356
!!   4444 Print*, "Int_step_P(Emin,Emax,5)"
!!   do ispin=1,NSpin
!!   
!!   PDMol = d_zero
!!   SDMol = SD(Mbeg:Mend,Mbeg:Mend)
!!   !SDMol = InvSD(Mbeg:Mend,Mbeg:Mend) !Dual
!!   !call RMatPow(SDMol,-1.0d0,SDMol)
!!   
!!   do i=Mbeg,Mend
!!   do j=Mbeg,Mend
!!       PDMol(i-Mbeg+1,j-MBeg+1) = PD(ispin,i,j)
!!   end do
!!   end do
...
!!       M=1000
!!   PDMol = SDMol
!!       call P_intpj_step(rrr,a,b,M,d_zero-dabs(biasvoltage/2.0),PDMol)
!!   PDMol = matmul(PDMol,SDMol)
!!   !PDMol = matmul(PDMol,SD(Mbeg:Mend,Mbeg:Mend))
!!   SDMol=0.0d0
!!   do i=Mbeg,Mend
!!   do j=Mbeg,Mend
!!   SDMol(i,i) = SDMol(i,i) + SD(i,j)*InvSD(j,i)
!!   end do
!!   end do
!!   do i=Mbeg,Mend
!!   do j=Mbeg,Mend
!!   Q = Q + PDMol(i,i)/RTrace(SDMol)
!!   end do
!!   end do
!!   !      Q = RTrace(PDMol)
!!   print*, "--------------------------------------"
!!   Print*, "P_intpj_step  = ", Q,RTrace(SDMol)
!!   print*, "--------------------------------------"
!!   end do
!!   
!!    1356   deallocate (PDMol)
!!       if (El1dim /= 0 ) deallocate (PDEl1)
!!       if (El2dim /= 0 ) deallocate (PDEl2)
!!   
!!   !    open(166, file=trim(jobname)//'.DENew.dat', status='unknown')
!!   
!!   !    WRITE(166,*) -Shift 
!!   !    WRITE(166,*) -ChemPot
!!   
!!   !    do i=1,NAORBS
!!   !      do j=1,NAORbs
!!   !        WRITE(166,'(2i6,e18.10)') i,j,PD(1,i,j)
!!   !        if (dabs(PD(1,i,j) - PD(1,j,i)) > 1.0d-05) print*, "DEN = ",
!!   !        i,j,PD(1,i,j),PD(1,j,i)
!!   !      end do
!!   !    end do
!!   
!!   !    close(166)
!!   
!!     end subroutine CompDensMat3
!!   
  ! 
